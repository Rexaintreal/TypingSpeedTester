...
#this line of code with give us sharper window
ctypes.windll.shcore.SetProcessDpiAwareness(1)

#Setup of customtkinter
root=customtkinter.CTk()

#dimensions of the window
root.geometry("1024x720")

#settig the font for all labels and buttons
root.option_add("*Label.Font", "consolas 30")
root.option_add("*Button.Font", "consolas 30")

heading=customtkinter.CTkLabel(root,text="Typing Speed Test", font=("",32)).place(relx=0.5,rely=0.3,anchor=tkinter.CENTER)

#function which contains practice text
def practiceTest():
    #practice lines list
    Alltexts=[
    ...
    #randomly chosing one text
    text=random.choice(Alltexts).lower()

    #deciding where to split the text
    split=0
    global labelleft
    labelleft=customtkinter.CTkLabel(root,text=text[0:split], fg_color='grey',font=("",28))
    labelleft.place(relx=0.5,rely=0.5,anchor=tkinter.E)

    #Text to be written
    global labelright
    labelright=customtkinter.CTkLabel(root, text=text[split:],font=("",28))
    labelright.place(relx=0.5,rely=0.5,anchor=tkinter.W)

    #letter pressed
    global currentletter
    currentletter=customtkinter.CTkLabel(root, text=text[split], fg_color='grey',font=("",28))
    currentletter.place(relx=0.5,rely=0.6, anchor=tkinter.N)

    #showing time left
    global timeleft
    timeleft=customtkinter.CTkLabel(root,text=f'0 seconds',fg_color='grey',font=("",28))
    timeleft.place(relx=0.5,rely=0.4,anchor=tkinter.S)
...
def stop():
    ...
    #Amount of words
    ammountWords=len(labelleft.cget('test').split(' '))
    ...
    global Result
    Result=customtkinter.CTkLabel(root, text=f'WPM (Words Per Minute): {ammountWords}',fg_color='black',font=("",28))
    Result.place(relx=0.5,rely=0.4,anchor=tkinter.CENTER)

    #Reset
    global Reset
    Reset=customtkinter.CTkButton(root, text=f'Retry', command=restart,font=("",28))
    Reset.place(relx=0.5, rely=0.6, anchor=tkinter.CENTER)
...
def addSecond():
    global passedSecond
    passedSecond+=1
    timeleft.configure(text=f'{passedSecond} Seconds')
...
def keyPress(event=None):
    try:
        if event.char.lower() == labelright.cget('text')[0].lower():
            ...
            labelleft.configure(text=labelright.cget('text')+event.char.lower())
            currentletter.configure(text=labelright.cget('text')[0])
